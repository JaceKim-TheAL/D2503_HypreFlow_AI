...
플로우 그래프를 실행합니다.
첫 번째 User input노드( getUserInput01)에서 PDF나 이미지 파일을 업로드하세요.
노드 Transform content( transformContent01)는 업로드된 파일에 대해 자동으로 OCR을 수행합니다.
OCR 프로세스가 완료되면 두 번째 User input노드( getUserInput02)로 이동하세요. "사용자 프롬프트" 필드에 문서에 대해 묻고 싶은 질문을 입력하세요.
그러면 노드 Call LLM with tools( toolAgentv201)가 추출된 텍스트와 질문을 사용하여 답변을 생성합니다.
답변은 노드의 출력 필드 중 하나 Call LLM with tools(예: Generated response또는 )에서 확인할 수 있습니다. HyperFlow BrowserTool output as text 에서 확인할 수 있습니다 .
다음 단계 / 확인:
